//
// Controller for SideScroller app
// - this app uses left/right arrow keys to "move" a player in a 
//   side scrolling game area. The player mostly stays in a position
...
public class SideScrollController extends AnimationTimer
{
...
public boolean jump, goLeft, goRight, running;
...
public void initialize()
{
   ...
   windowPane.setOnKeyReleased(new EventHandler<KeyEvent>() {
    ...
    Rectangle clip = new Rectangle(900, 640);
    ...
    redBall.setCenterX(70); redBall.setCenterY(580);
    // create the background -- checker board of greyish rectangles
    double grayLevel = 0.5;
    for (int x=0; x < 7000; x+=200) {
       for (int y=0; y < 638; y+=213) {
          Rectangle r = new Rectangle(200, 213, Color.color(grayLevel, 
...
public void handle(long now)
{
    ...
    if (timerBasedMovement) {
       ...
       //if (false) posAdjustY = dirRate;
       //else if (jump) posAdjustY = -dirRate;
       //else posAdjustY = 0;
          posAdjustY = 0;
       if (posAdjustY > 0 && redBall.getCenterY() + redBall.getRadius() > gamePane.getHeight())
           ...
           posAdjustY = 0;
       if (goLeft) posAdjustX = -dirRate;
       else if (goRight) posAdjustX = dirRate;
       else posAdjustX = 0;
